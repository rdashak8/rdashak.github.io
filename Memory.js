$(document).ready(function(){
    var cardWidth;localStorage.clear();
    var cardHeight;
    var size;
    //проверка на ту же самую карту
    var flag;
    var ident_of_timer;
    var timer;
    var level;
    ...
    function StartGame(xmlDom){
        ...
        $("#startGame").append("<br />"+$(xmlDom).find("about").text()+"<br />"+"<br />");
    ...
    function BeginLevel(number,widt,heig,timee){
       ...
       $("#helpBut").show();
       $("#nextBut").hide();
       if (localStorage.getItem(level)!=null) $("#nextBut").show();
       //номера карт
       var k=-1;
                ...
                if (flag){
                    ...
                    //если две открытые картинки
                        ...
                        if (m[0].attr("id")!=m[1].attr("id")){
                            ...
                            //бонусы
                            if (pair_next){
                                var a=(Number)(localStorage.getItem("forhelp"));
                                localStorage.setItem("forhelp",a+1);
                            } 
                            pair_next=true;
    ...
    function CheckAllPairs(){
        if ($(".disable").length==size){
            ...
            localStorage.setItem(level,point);
    ...
    $("#helpBut").click(function(){
        ...
        $("#make_pair").click(function(){
            CloseHelpWindow();
            var a=localStorage.getItem("forhelp");
            localStorage.setItem("forhelp",a-1);
            $(".pict").each(function(){
                if (!$(this).hasClass("disable")){
                    var id=$(this).attr("id");
    ...
    function OpenHelpWindow(){
        $("#info").show();
        $("#info").append("<p id='showPointsForHelp'><u>Бонусы: "+(Number)(localStorage.getItem("forhelp")));
        if (localStorage.getItem("forhelp")>0) $("#info").append("<input type='button' id='make_pair' value='Использовать'>");
    ...
    function End_All_Levels(){
        ...
        $("#tableInf").append($("<tr><th>Имя</th><th>Уровень</th><th>Очки</th></tr>"));
        for(i=1; i<localStorage.length; i++) {
            $("#tableInf").append($("<tr><td>"+userNameInGame+"</td><td>"+i+"</td><td>"+localStorage.getItem(i)+"</td></tr>"));
        }
